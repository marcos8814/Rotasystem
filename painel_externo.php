        ...
        <div class="form-area">
          <?php  
                  ...
                  if (isset($_POST['logar']))
                  {
                    ...
                    $selecionarota=$con->prepare("SELECT * FROM rota WHERE rot_rfid =:rot_rfid AND rot_status_rota !='Finalizada';");
                        ...
                        if ($linha["rot_status_rota"] =="Inativo") 
                          {
                            ...
                            $busca = $con->prepare("UPDATE  rota SET  rot_horaeatrazo =:rot_horaeatrazo, rot_status_rota = 'Em rota' WHERE rot_rfid =:rot_rfid AND rot_status_rota !='Finalizada';");
                            $busca->bindParam(':rot_rfid', $rfid, PDO::PARAM_STR);
                            $busca->bindParam(':rot_horaeatrazo', $hora, PDO::PARAM_STR);
                            $busca->execute();
                            $busca-> rowCount();
                          ...
                          {
                            //ATUALIZA O CAMPO rot_horaeatrazo DE ENTRADA E MUDA O rot_status_rota PARA: EM ROTA 
                            $busca = $con->prepare("UPDATE  rota SET  rot_horasatrazo =:rot_horasatrazo, rot_status_rota = 'Finalizada' WHERE rot_rfid =:rot_rfid AND rot_status_rota !='Finalizada';");
                            $busca->bindParam(':rot_rfid', $rfid, PDO::PARAM_STR);
                            $busca->bindParam(':rot_horasatrazo', $hora, PDO::PARAM_STR);
                            $busca->execute();
                            $busca-> rowCount();

                            $atulizavei = $con->prepare("UPDATE  veiculos SET  vei_status ='Disponivel' WHERE vei_etiqueta =:vei_etiqueta;");
                            $atulizavei->bindParam(':vei_etiqueta', $rfid, PDO::PARAM_STR);
                            $atulizavei->execute();
          ...
          ?>
          <div class="group">
            <input type="text" name="rfid" class="form-control" placeholder="" autofocus>
